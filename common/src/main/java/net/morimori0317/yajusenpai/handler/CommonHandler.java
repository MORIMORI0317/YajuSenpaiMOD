...
public class CommonHandler {
        ...
        if (!livingEntity.level().isClientSide()) {
            ...
            if (livingEntity.hasEffect(YJMobEffects.IKISUGI.get()) && effect != null) {
                if (effect.getDuration() <= 20) {
                    ...
                    livingEntity.hurt(YJDamageSources.ikisugi(livingEntity.level(), yjLiving.getGrantedIkisugiEntity(), null), 114514f);
                }
    ...
    public static void onLivingDrop(LivingEntity livingEntity, DamageSource source, Collection<ItemEntity> drops) {
        if (livingEntity.getMaxHealth() >= 100 && YJUtils.isIkisugiDamage(source)) {
